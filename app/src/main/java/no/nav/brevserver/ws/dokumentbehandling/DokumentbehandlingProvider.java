...
public class DokumentbehandlingProvider {
	...
	public HentDokumentResponse2 hentDokument(HentDokumentRequest request) throws BrevTechnicalException, BrevFunctionalException, BrevFinnesIkkeException {
		no.nav.brevserver.ws.dokumentbehandling.to.HentDokumentRequest hentDokumentRequest = hentDokumentRequestMapper.map(request);
	...
	public void lagreDokument(LagreDokumentRequest lagreDokumentRequest) throws BrevException {
		no.nav.brevserver.ws.dokumentbehandling.to.LagreDokumentRequest request = lagreDokumentRequestMapper.map(lagreDokumentRequest);
		request.validate();
	...
	public void avbrytDokument(AvbrytDokumentRequest avbrytDokumentRequest) throws BrevException {
		no.nav.brevserver.ws.dokumentbehandling.to.AvbrytDokumentRequest request = avbrytDokumentRequestMapper.map(avbrytDokumentRequest);
		request.validate();
		BrevStatusVO brevStatus = request.getBrevStatus();
	...
	public void ferdigstillDokument(FerdigstillDokumentRequest ferdigstillDokumentRequest) throws BrevException {
		no.nav.brevserver.ws.dokumentbehandling.to.FerdigstillDokumentRequest request = ferdigstillDokumentRequestMapper.map(ferdigstillDokumentRequest);
		request.validate();
